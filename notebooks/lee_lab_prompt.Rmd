...
LinearModels <- function(data, x, y) {
    ...
    co2_lm_poly <- lm(as.formula(paste(y, "~ poly(",x,", degree = 2, raw = TRUE)")), data = data)
...
models <- LinearModels(co2_ts, 'index', 'value')
```
...
co2_lm_forecast %>%
    autoplot() +
    geom_line(aes(y = co2_ts$value, x = co2_ts$index), color = "red") +
    labs(
...
```{r forecast models}
co2_lm_quad <- lm(value ~ t + I(t^2), data = co2_ts)

co2_lm_quad_plot <- co2_ts %>%
    ggplot(aes(x = index, y = value)) +
    geom_line(color = 'steelblue') +
    geom_line(aes(y = predict(co2_lm_quad, co2_ts)), color = 'red') +
    labs(
        title = TeX(r'(Monthly Mean $CO_2$ fitted with quadratic trend)'),
        x = 'Month and Year',
        y = TeX(r'($CO_2$ parts per million)')
...
```{r forecast models}
lm_quad_model <- co2_ts %>% model(lm = TSLM(value ~ poly(index, 2)))
co2_lm_quad_forecast <- forecast(lm_quad_model, h = h_calc)

co2_lm_quad_forecast %>%
    autoplot() +
    geom_line(aes(y = co2_ts$value, x = co2_ts$index), color = "red") +
    labs(
        title = TeX(r'(Monthly Mean $CO_2$ forecast)'),
...
# accuracy(co2_forecast, co2_monthly_ts %>% mutate(value = average, index = ym))
...
```{r present vs past forecast atmospheric CO2 growth, echo=F, warning=F, message=F, fig.align='center'}
# plot forcast and actual values from co2_ts
co2_forecast[0:480,] %>%
    autoplot() +
...
```{r present month-average series, echo=F, warning=F, message=F}
...
```{r present month-average forecast linear model test, echo=F, warning=F, message=F, fig.align='center'}
accuracy(co2_lm_forecast, co2_monthly_ts %>% mutate(value = average, index = ym))[3:4] %>% kable()
...
```{r present month-average forecast poly model test, echo=F, warning=F, message=F, fig.align='center'}
accuracy(co2_lm_quad_forecast, co2_monthly_ts %>% mutate(value = average, index = ym))
...
```{r present month-average ARIMA test, echo=F, warning=F, message=F, fig.align='center'}
co2_forecast %>% accuracy(co2_monthly_ts %>% mutate(value = average, index = ym))[3:4] %>% kable()
```
...
# split_date <- max(co2_monthly_ts$ym) - 24   # 2 * 12months
# train_data <- co2_monthly_ts %>% filter(ym <= split_date)
# test_data <- co2_monthly_ts %>% filter(ym > split_date)
...
# train_data <- train_data %>% left_join(components, by = "time_index", suffix = c("", "_season"))
...
nsa_test_accuracy <- nsa_forecast %>% accuracy(test_data)
nsa_train_accuracy <- nsa_forecast %>% accuracy(co2_present_ts)
# sa_train_accuracy <- sa_forecast %>% accuracy(test_data)
# train_data
```

```{r}
# Plot the training data and the forecast for the NSA model
nsa_forecast %>%
    autoplot(test_data) +
    labs(
...
# Plot the training data and the forecast for the SA model
co2_test_decomp <- test_data %>% model(STL(average ~ season(window = 12)))
test_data_season <- test_data %>% mutate(season_adjust = components(co2_test_decomp)$season_adjust)
test_data_avg <- test_data %>% mutate(season_adjust = average)
sa_forecast %>%
    autoplot(test_data_season) + # replace with test_data_season or test_data_avg
    labs(
...
co2_lm_poly2 <- lm(average ~ poly(time_index, degree = 2, raw = TRUE), data = test_data)

# Plot the co2_lm_poly2 against the test_data_avg
test_data %>%
    ggplot(aes(x = time_index, y = average)) +
    geom_line() +
    geom_line(aes(y = predict(co2_lm_poly2, test_data)), color = "red") +
    labs(
        title = "[test, average] Polynomial Time-Trend Model",
        x = "Year",
        y = "CO2 Levels (ppm)"
    )

co2_lm_poly3 <- lm(season_adjust ~ poly(time_index, degree = 2, raw = TRUE), data = train_data_season)
train_data_season %>%
    ggplot(aes(x = time_index, y = season_adjust)) +
    geom_line() +
    geom_line(aes(y = predict(co2_lm_poly3, train_data_season)), color = "red") +
    labs(
        title = "[train, season_adjust] Polynomial Time-Trend Model",
        x = "Year",
        y = "CO2 Levels (ppm)"
    )
```
...
```{r}
forecast_7800 <- forecast(nsa_model, h=7800)
forecast_7800 %>% autoplot(co2_present_ts)

na_2100_mean <- forecast_7800$.mean[h_calc]
jan_2100_dist <- forecast_7800$value[h_calc]
jan_2100_dist

first_420_mean <- forecast_7800 %>% filter(.mean > 420)
first_420_mean$index[1]


fc_hilo <- forecast_7800 %>% hilo() %>% unpack_hilo('95%')
fc_hilo <- fc_hilo %>%
    rename(replace=c('95%_upper' = 'upper_95')) %>%
    rename(replace=c('95%_lower' = 'lower_95'))
...
print(paste("Forecast first crosses 420 at time", crosses_420_time))
print(paste("Forecast first crosses 500 at time", crosses_500_time))
print(paste("Forecast last crosses 420 at time", crosses_last_420_time))
print(paste("Forecast last crosses 500 at time", crosses_last_500_time))
forecast_2122 <- subset(longForecast, year(time_index) == 2122)
# Calculate the average of the 'average.y' column for the year 2021
average_2122 <- mean(forecast_2122$average)
forecast_2122$average
# Print the average value
mean(average_2122)



first_420_mean <- min(co2_forecast$time_index[co2_model$.mean >= 420])
co2_forecast
```

```{r}
fc_hilo <- longForecast %>%
    hilo() %>%
    unpack_hilo('95%')
...
first_420_upper <- fc_hilo %>% filter(upper_95 > 420)
first_420_upper$time_index[1]

last_420_lower <- fc_hilo %>% filter(lower_95 > 420)
last_420_lower$time_index[1]

# 500 ppm
first_500_upper <- fc_hilo %>% filter(upper_95 > 500)
first_500_upper$time_index[1]

last_500_lower <- fc_hilo %>% filter(lower_95 > 500)
last_500_lower$time_index[1]
# confidence interval
conf_int <- last_500_lower %>% select(lower_95, upper_95)
conf_int
```