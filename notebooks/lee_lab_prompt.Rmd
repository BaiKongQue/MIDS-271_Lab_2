...
```{r present month-average series}
...
```{r present month-average forecast linear model test}
LinearModels(co2_monthly_ts, "ym", "average")
```

```{r present month-average ARIMA test}
co2_monthly_arima_model <- co2_monthly_ts %>% model(ARIMA(average ~ 1 + pdq(0:10,0:3,0:10) + PDQ(0:10,0:3,0:10), ic="bic", stepwise=F, greedy=F))
co2_present_model %>% report()
co2_present_model %>% augment() %>% ACF(.resid) %>% autoplot()
```
...
co2_decomposition <- train_data %>% model(STL(average ~ season(window = 12)))
...
nsa_model <- train_data %>%
  model(ARIMA(average ~ pdq(0:10,0:3,0:10) + PDQ(0:10,0:3,0:10)))
...
sa_forecast %>%
    autoplot(test_data_avg) + # replace with test_data_season or test_data_avg
    labs(
        title = "[average] Forecast of CO2 Levels (SA)",
        x = "Year",
        y = "CO2 Levels (ppm)"
    )
```
