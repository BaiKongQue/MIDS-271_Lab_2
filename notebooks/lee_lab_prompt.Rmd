...
```{r present month-average ARIMA test}
forecast <- co2_monthly_arima_model %>% forecast(h = "2 years")
co2_forecast %>% accuracy(co2_monthly_ts %>% mutate(value = average, index = ym))
```
...
nsa_model <- train_data %>%
  model(ARIMA(average ~ 1 + pdq(0:10,0:3,0:10) + PDQ(0:10,0:3,0:10), ic="aic", stepwise=F, greedy=F))
