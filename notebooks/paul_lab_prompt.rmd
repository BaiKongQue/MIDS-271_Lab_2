...
co2_ts <- as_tsibble(co2) %>%
  mutate(t = co2_ts$index - co2_ts$index[1]) %>%
...
gg_season(data=co2_ts, y=ln_val)
...
m_lin <- lm(formula = ln_val ~ t, data = co2_ts)
m_quad <- lm(formula = ln_val ~ I(t^2) + t, data = co2_ts)
m_pol <- co2_ts %>% model(lm = TSLM(ln_val ~ t + I(t^2) + trend() + season()))


m_lin_res <- m_lin$residuals
m_quad_res <- m_quad$residuals

lin_plot <- co2_ts %>% 
  ggplot() + 
  aes(x=index, y=ln_val) + 
  geom_line(color = 'steelblue') +
  geom_line(aes(x = index, y= m_lin$fitted.values), color = "red") +
  labs(
    title = TeX(r'(Monthly Mean $CO_2$ with Linear Trend)'),
    x = 'Month and Year',
...
quad_plot <- co2_ts %>% 
  ...
  aes(x=index, y=ln_val) + 
  geom_line(color = 'steelblue') +
  geom_line(aes(x = index, y= m_quad$fitted.values), color = "red") +
  labs(
    title = TeX(r'(Monthly Mean $CO_2$ with Quadratic Trend)'),
    x = 'Month and Year',
    y = TeX(r'(Log of $CO_2$ parts per million)')
  )
...
poly_plot <- co2_ts %>% 
  ...
  aes(x=index, y=ln_val) + 
  geom_line(color = 'steelblue') +
  geom_line(aes(x = index, y= m_pol$fitted.values), color = "red") +
  labs(
    title = TeX(r'(Monthly Mean $CO_2$ with Quadratic Trend)'),
    x = 'Month and Year',
    y = TeX(r'(Log of $CO_2$ parts per million)')
...
lin_plot | quad_plot | poly_plot

m_pol %>% gg_tsresiduals()
...
acf(co2) 
pacf(co2)
tseries::adf.test(co2, alternative = "stationary")
PP.test(co2)
?adf.test

model_bic <- co2_ts %>%
  model(ARIMA(value ~ 1 + pdq(0:10,0:2,0:10) + PDQ(0:10,0:2,0:10), ic="bic", stepwise=F, greedy=F))

model_bic %>% report()

model_bic %>% augment() %>% ACF(.resid) %>% autoplot()

```

Box Jenkins:
 - Determine if AR, MA, ARMA, or ARIMA is appropriate
 - Determine th eparameters pd, d, and q using a combination of ACF, PACF, AIC, BIC, etc
 - Fit the model and examine residuals, and use a Ljung Box test for autocorrelation
...
forecast_1997 <- forecast(model_bic, h=h_calc)

forecast_1997 %>% autoplot(co2_ts)

na_2100_mean <- forecast_1997$.mean[h_calc]
jan_2100_dist <- forecast_1997$value[h_calc]
jan_2100_dist

first_420_mean <- forecast_1997 %>% filter(.mean > 420)
first_420_mean$index[1]

"OK, I can read the friggin' Normal parameters, but how do I extract them? If I extract them, I can add a low and high confidence interval to the forecasts and find the first and last instance where the intervale crosses 420"
forecast_1997$value[1]
```
